...
# ⚠️ আপনার বটের আসল API Token-টি অবশ্যই উদ্ধৃতি চিহ্নের (" ") ভেতরে দিন
BOT_TOKEN = "8895135409:AAHpo18y1o74_g1XBeTMO7CCpjj0NYfjWHA"

# ডাটাবেজের বিকল্প হিসেবে সাময়িক ডেটা স্টোরেজ (In-Memory Data)
user_data_store = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if referrer_id != user.id and referrer_id in user_data_store:
                ...
                await context.bot.send_message(
                    chat_id=referrer_id, 
                    text="🎉 আপনার রেফারাল লিংকের মাধ্যমে একজন নতুন ইউজার যুক্ত হয়েছেন! আপনি ২০০ কয়েন বোনাস পেয়েছেন।"
                )
        except ValueError:
            pass
    ...
    await update.message.reply_text(
        f"👋 হে {user.first_name}, স্বাগতম!\n\nনিচের মেনু থেকে আপনার পছন্দ অনুযায়ী অপশন নির্বাচন করুন:",
...
async def add_channel_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("আপনার চ্যানেলের লিংক বা ইউজারনেমটি সেন্ড করুন (তার আগে বটকে এডমিন করুন):")
...
async def set_emoji_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("রিয়্যাকশন ইমোজিগুলো লিখে পাঠান (যেমন: 👍 ❤️ 🔥):")
    return WAITING_FOR_EMOJIS

async def set_emoji_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u_data = get_user_data(update.effective_user.id)
    u_data['emojis'] = update.message.text
...
async def set_count_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("একটি পোস্টে কতগুলো রিয়্যাকশন চান তার সংখ্যা লিখুন:")
...
async def set_count_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ শুধু সংখ্যা টাইপ করুন।")
    return ConversationHandler.END
